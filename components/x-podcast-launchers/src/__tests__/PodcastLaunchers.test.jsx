...
describe('PodcastLaunchers', () => {
	it('should hide itself if an RSS URL could not be generated', () => {
		expect(
			renderer
				.create(<PodcastLaunchers conceptId="123-abc" {...{ acastRSSHost, acastAccessToken }} />)
				.toJSON()
		).toMatchSnapshot()
	})
	...
	it('should render the app links based on concept Id', () => {
		expect(
			renderer
				.create(
					<PodcastLaunchers conceptId={brand.rachmanReviewPodcast} {...{ acastRSSHost, acastAccessToken }} />
				)
				.toJSON()
		).toMatchSnapshot()
	})
