...
const noAppWrapperStyles = ['podcast-launchers__no-app-wrapper', styles.noAppWrapper].join(' ')
				...
				<div className={styles.container} data-trackable="podcast-launchers">
					...
					<ul className={styles.podcastAppLinksWrapper}>
						{generateAppLinks(rssUrl).map(({ name, url, trackingId }) => (
							<li key={name}>
								<a href={url} className={styles.podcastAppLink} data-trackable={trackingId}>
									{name}
								</a>
							</li>
						))}
